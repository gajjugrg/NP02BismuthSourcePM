...
def process_directory(directory):
    ...
    # CHANGED: Create a mask to remove NaN/Inf values
    valid_mask = np.isfinite(voltage) & np.isfinite(signal_subtracted)  # CHANGED
    if not np.any(valid_mask):  # CHANGED
        print("No valid data points after removing NaNs or Infs.")
        return

    # CHANGED: Use clean arrays for curve fitting
    voltage_clean = voltage[valid_mask]
    signal_subtracted_clean = signal_subtracted[valid_mask]

    # Initial guess for Gaussian fit
    initial_guess = [np.nanmax(signal_subtracted_clean), voltage_clean[np.nanargmax(signal_subtracted_clean)], 0.05]  # CHANGED

    try:
        # CHANGED: Use the clean arrays for fitting
        popt, _ = curve_fit(gaussian, voltage_clean, signal_subtracted_clean, p0=initial_guess)
    except Exception as e:
    ...
    voltage_fit = np.linspace(np.nanmin(voltage_clean), np.nanmax(voltage_clean), 1000)  # CHANGED
    ...
    plt.subplot(2, 1, 2)
    plt.plot(voltage_clean, signal_subtracted_clean, label='Background-Subtracted Signal', color='red')  # CHANGED
